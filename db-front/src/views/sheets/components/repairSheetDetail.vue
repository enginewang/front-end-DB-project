<template>
  <page-view title="报修单"
    ...
    <a-card :bordered="false"
      ...
      <a-steps :direction="isMobile() && 'vertical' || 'horizontal'"
               :current="this.details.state-'0'"
               progressDot>
        <a-step title="用户提交">
        </a-step>
        <a-step title="巡检员提交">
    ...
    <a-card :bordered='false'
      ...
      <a-row grid
        ...
        <a-row>
          <a-col :span='8'>
            <a-form-item :validate-status="successRepair"
              ...
              <a-select :disabled="qualified"
                ...
                <a-select-option v-for="id in this.stfList"
                                 :key="id.staffId">
                  {{id.staffName}}
                </a-select-option>
        ...
        <a-row>
          <a-col :span='8'>
            <a-form-item label="器材选择"
              ...
              <a-select :disabled="qualifiedForEq"
                ...
                <a-select-option v-for="eq in this.eqType"
                                 :key="eq.no">
                  {{eq.type+' '+eq.model}}
                </a-select-option>
              </a-select>
          ...
          <a-col :span='6'
                 style="">
            <a-button :disabled="qualifiedforButton"
                      type='primary'
                      @click="handleAddEq">添加</a-button>
          </a-col>
      ...
      <a-row grid
        ...
        <a-row>
          <a-col :span='8'>
            <a-form-item label="配件选择"
              ...
              <a-select :disabled="qualified"
                ...
                <a-select-option v-for="ac in this.acType"
                                 :key="ac.no">
                  {{ac.type+' '+ac.model}}
                </a-select-option>
          ...
          <a-col :span='9'>
            <a-form-item :validate-status="compareAc"
                         label="数量"
                         :label-col="{ span: 5 }"
                         :wrapper-col="{ span: 18 }">
          ...
          <a-col :span='6'>
            <a-button :disabled="qualifiedforButtonAc"
                      type='primary'
                      @click="handleAddAc">添加</a-button>
          </a-col>
        </a-row>
      </a-row>
      <div :span='9'
           style="margin-bottom:24px">
        <a-button :disabled="qualifiedforSubmit"
                  type='primary'
                  @click="onClickSubmit">提交</a-button>
        <a-modal title="确认提交"
                 v-model="visible"
                 @ok="handleOK">
          <div class="modal">
      ...
      <a-table :columns="columns"
               :dataSource="this.result.ls"
               bordered>
        <template v-for="col in ['statue','type','model', 'number']"
                  :slot="col"
                  slot-scope="text">
          <div :key="col">
            {{ text }}
